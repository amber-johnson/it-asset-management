...
from .app_settings import SHIB_ATTRIBUTE_MAP, GROUP_ATTRIBUTES, GROUP_DELIMITERS
...
class ShibbolethRemoteUserMiddleware(RemoteUserMiddleware):
    ...
    def process_request(self, request):
        if not request.META.get(self.header): return

        # Make sure we have all required Shiboleth elements before proceeding.
        shib_meta, error = self.parse_attributes(request)

        username = shib_meta.get('username')
        ...
        user = auth.authenticate(
            request, remote_user=username, shib_meta=shib_meta)
